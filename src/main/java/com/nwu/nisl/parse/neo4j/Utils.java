...
//                System.out.println(methodCallExpr.getNameAsString() + "：是系统、库函数");
    ...
    /**
     * @Author:yueyue on 2020/12/10 20:53
     * @Param:
     * @return:获取文件所在项目的相对路径（从所在项目根目录开始）
     * @Description: 确保路径中含有source字段
     */
    public static String getFileNameWithPath(File file) {
        String res = file.getPath();

        String pathwithVersion = res.substring(res.indexOf("source") + "source".length() + 1);
        String[] array = pathwithVersion.split(Matcher.quoteReplacement(File.separator));
        String version = array[0];
        String Rtpath = pathwithVersion.substring(pathwithVersion.indexOf(version) + version.length() + 1).replace("\\", "/");
        return Rtpath;
    }

    /**
     * @Author:yueyue on 2020/12/10 21:03
     * @Param: 根据文件的绝对路径获取版本号
     * @return: 版本号
     * @Description:
     */
